/* JerryFX - A Chess Graphical User Interface with repertoire builder
...
public class RepertoireBoard extends Canvas {
    ...
    public RepertoireBoard(Board board) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                if(piece != EMPTY && piece != FRINGE) {
			      ...
			      j == grabbedPiece.sourceSquareY())) {
                            Image pieceImage = pieceImageProvider.getImage(piece,
    ...
    void handleMouseRelease(MouseEvent e) {
        ...
        if (clickedOnBoard(e.getX(), e.getY())) {
            Point boardPos = getBoardPosition(e.getX(), e.getY());
            if (grabbedPiece.isGrabbed()) {
                if ((boardPos.x != grabbedPiece.sourceSquareX() || boardPos.y != grabbedPiece.sourceSquareY())) {
